#include "stdafx.h"
#include "..\Public\Monster_Purple_Apple.h"
...
CMonster_Purple_Apple::CMonster_Purple_Apple(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CMonster{ pGraphic_Device }
{
}

CMonster_Purple_Apple::CMonster_Purple_Apple(const CMonster_Purple_Apple& Prototype)
	:CMonster{ Prototype }
{
}


HRESULT CMonster_Purple_Apple::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMonster_Purple_Apple::Initialize(void* pArg)
{
	if (nullptr == pArg)
		return E_FAIL;
	...
	m_tActorDesc.iHp = 15;
	m_pAnimatorCom->Change_Texture(TEXT("Monster_Purple_Apple_Move"));
	...
	m_pColliderCubeCom->Scaling(.3f, 1.f, .3f);
	return S_OK;
}

_uint CMonster_Purple_Apple::Priority_Update(_float fTimeDelta)
{
...
void CMonster_Purple_Apple::Update(_float fTimeDelta)
{

	m_Calculate_Range += 0.05f;
	if (m_Calculate_Range >= 10.f)
		m_Calculate_Range = 1.f;
	...
	{
		m_pAnimatorCom->Change_Texture(TEXT("Monster_Purple_Apple_Die"));
		m_bDeadMotion = true;
		m_iFrameCounter = 0;
	}
	...
	__super::SetUp_OnTerrain(m_pTransformCom, 0.5);
}

void CMonster_Purple_Apple::Late_Update(_float fTimeDelta)
{
...
HRESULT CMonster_Purple_Apple::Render()
{
...
HRESULT CMonster_Purple_Apple::OnCollision(CGameObject* pOther, const _wstring& strLayerTag)
{
...
HRESULT CMonster_Purple_Apple::Find_Enemy()
{
	_float fResult, fr;
...
HRESULT CMonster_Purple_Apple::Ready_Components() 
{
	/* For.Com_Animator*/
...
//Texture
	...
	m_pAnimatorCom->Add_Texture(TEXT("Monster_Purple_Apple_Move"),
		static_cast<CTexture*>(m_pGameInstance->Clone_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_Monster_Purple_Apple_move"))));

	m_pAnimatorCom->Add_Texture(TEXT("Monster_Purple_Apple_Die"),
		static_cast<CTexture*>(m_pGameInstance->Clone_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_Monster_Purple_Apple_Die"))));
...
HRESULT CMonster_Purple_Apple::SetUp_RenderState()
{
...
HRESULT CMonster_Purple_Apple::Reset_RenderState()
{
	m_pGraphic_Device->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATE);
...
CMonster_Purple_Apple* CMonster_Purple_Apple::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	
	CMonster_Purple_Apple* pInstance = new CMonster_Purple_Apple(pGraphic_Device);
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CMonster_Purple_Apple"));
...
CGameObject* CMonster_Purple_Apple::Clone(void* pArg)
{
	CMonster_Purple_Apple* pInstance = new CMonster_Purple_Apple(*this);
	
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CMonster_Purple_Apple"));
		Safe_Release(pInstance);
	}
...
void CMonster_Purple_Apple::Free()
{
	__super::Free();
