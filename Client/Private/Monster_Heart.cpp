#include "stdafx.h"
#include "..\Public\Monster_Heart.h"
...
CMonster_Heart::CMonster_Heart(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CMonster{ pGraphic_Device }
{
}

CMonster_Heart::CMonster_Heart(const CMonster_Heart& Prototype)
	:CMonster{ Prototype }
{
}

HRESULT CMonster_Heart::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMonster_Heart::Initialize(void* pArg)
{
	if (nullptr == pArg)
		return E_FAIL;
	// 0624 심현우
	...
	m_tActorDesc.iHp = 15;
	m_pAnimatorCom->Change_Texture(TEXT("Monster_Heart_Move"));
	...
	m_pColliderCubeCom->Scaling(.4f, 1.f, .4f);
	return S_OK;
}

_uint CMonster_Heart::Priority_Update(_float fTimeDelta)
{
...
void CMonster_Heart::Update(_float fTimeDelta)
{

	Calculate_Range += 0.05f;
	if (Calculate_Range >= 100.f)
		Calculate_Range = 1.f;
	...
	{	
		m_pAnimatorCom->Change_Texture(TEXT("Monster_Heart_Die"));
		m_bDeadMotion = true;
		m_iFrameCounter = 0;
	}
	...
	__super::SetUp_OnTerrain(m_pTransformCom, .5f);
}

void CMonster_Heart::Late_Update(_float fTimeDelta)
{
...
HRESULT CMonster_Heart::OnCollision(CGameObject* pOther, const _wstring& strLayerTag)
{
...
HRESULT CMonster_Heart::Find_Enemy()
{
	_float fResult, fr;
...
HRESULT CMonster_Heart::Ready_Components() 
{
	/* For.Com_Animator*/
...
//Texture
	...
	m_pAnimatorCom->Add_Texture(TEXT("Monster_Heart_Move"),
		static_cast<CTexture*>(m_pGameInstance->Clone_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_Monster_Heart_Move"))));

	m_pAnimatorCom->Add_Texture(TEXT("Monster_Heart_Die"),
		static_cast<CTexture*>(m_pGameInstance->Clone_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_Monster_Heart_Die"))));
...
HRESULT CMonster_Heart::SetUp_RenderState()
{
...
HRESULT CMonster_Heart::Reset_RenderState()
{
	m_pGraphic_Device->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATE);
...
CMonster_Heart* CMonster_Heart::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	
	CMonster_Heart* pInstance = new CMonster_Heart(pGraphic_Device);
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CMonster_Heart"));
...
CGameObject* CMonster_Heart::Clone(void* pArg)
{
	CMonster_Heart* pInstance = new CMonster_Heart(*this);
	
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CMonster_Heart"));
		Safe_Release(pInstance);
	}
